...
class Bank:
    ...
    def withdraw(self,amount):
        temp = db_query(f"SELECT balance FROM customers WHERE username = '{self.__username}'")
        if amount>temp[0][0]:
            print("Insufficient balance .Please deposit your money")
        else:
    ...
    def fundtransfer(self,receive,amount):
        ...
        if amount>temp[0][0]:
            ...
            temp2 = db_query(f"SELECT balance FROM customers WHERE username = '{self.__username}'")
            test1 = temp[0][0] - amount 
            test2 = temp[0][0] + amount 
            db_query(f"UPDATE customers SET balance = '{test1}' WHERE username = '{self.__username}';")
            db_query(f"UPDATE customers SET balance = '{test1}' WHERE account_number = '{receive}';")
